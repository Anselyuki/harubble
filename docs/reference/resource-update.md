...
- **发布模式**：应用启动后从仓库 `main` 分支拉取注册表，并把 `updatedAt` 当作不透明版本标记与本地缓存比较；值不同时会先原子写入缓存，再替换内存数据。实现不解析时间先后，因此发布时必须保证该值随内容更新。
- **开发模式**：启动时读取工作区的 `data/tag_registry.json`，便于直接验证本地修改。
- **播放退让**：启动后的同步任务在拉取本地/远端数据前等待 `PlaybackLoadGate`，避免与播放启动争用普通 API 和索引资源。
- **搜索索引**：注册表变化少于 50 个专辑时优先增量刷新；无活跃索引、增量失败或变化规模较大时回退到全量重建。
- **本地编辑**：Tag Editor 修改实体标签后异步增量刷新受影响专辑；无活跃索引时跳过，拉取或写入失败只记录日志，等待下次全量重建修复。
